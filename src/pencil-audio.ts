...
export function playStroke(elementType: string): void {
  ...
  // Per-type gain: shapes most prominent, text medium, arrows lighter
  const typeGain = isLine ? 1.0 : isText ? 2.0 : 2.5;
  const baseVolume = (0.4 + Math.random() * 0.3) * typeGain;
  gain.gain.setValueAtTime(0, ctx.currentTime);
  gain.gain.linearRampToValueAtTime(baseVolume, ctx.currentTime + 0.03);
